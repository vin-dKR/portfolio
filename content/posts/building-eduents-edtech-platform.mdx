...
- [What Eduents is](#what-eduents-is)
- [What I own vs what my teammate built](#what-i-own-vs-what-my-teammate-built)
- [Why five small repos, not one big one](#why-five-small-repos-not-one-big-one)
- [The database design](#the-database-design)
- [Reordering questions without breaking everything](#reordering-questions-without-breaking-everything)
- [The PDF maker](#the-pdf-maker)
- [Math on screen vs math in the PDF](#math-on-screen-vs-math-in-the-pdf)
- [Live collaboration without scary bugs](#live-collaboration-without-scary-bugs)
- [One middleware, three jobs](#one-middleware-three-jobs)
- [Reading questions from PDFs](#reading-questions-from-pdfs)
- [OMR grading](#omr-grading)
- [Cross-app calls](#cross-app-calls)
- [Bugs that ate days](#bugs-that-ate-days)
- [Things I want to fix next](#things-i-want-to-fix-next)
...
I built the main app and the live-collaboration server. My teammate built the Python tools that read PDFs and detect images. <span className="underline-em">We meet at a small set of API rules</span>.
...
One choice was a bit unusual - <span className="squiggly">a `Student` is **not** a `User`</span>. A student who only takes one OMR exam should not have to sign up. So the exam side has its own `Student` model with no login link. Cleaner, fewer accounts to manage.
...
I used a trick called <span className="underline-em">"fractional positions"</span>. Instead of integer positions (1, 2, 3...), I use floats (1.0, 2.0, 3.0...). To put a question between 2.0 and 3.0, I just save 2.5. No other rows change. To put one between 2.0 and 2.5, save 2.25. And so on.
...
**Problem 2 - "query engine not found" on Vercel.** Prisma needs a small binary file to talk to the database. With my custom Prisma setup, <span className="squiggly">that file did not get copied to the deploy bundle</span>. The deploy died.
...
The server is <span className="underline-em">dumb on purpose</span>. It just passes messages around. It does **not** save anything to the database. All saving is done by normal server actions in the main Next.js app.
...
Step 2 was <span className="squiggly">the bug that ate the most time</span>. Vision models silently rotated the image. My cropping code did not. So the boxes pointed to the wrong place. Fixing it once, in one place, fixed the bug for the whole pipeline.
...
- <span className="underline-em">I can own a full Next.js + Prisma + MongoDB system</span> - schema, middleware, server actions, PDF pipeline, live collab.
